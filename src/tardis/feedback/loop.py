...
class FeedbackLoop:
    ...
    def add_from_autopsy(
        self, trace_id: str, autopsy_result: dict, trace_steps: list[Any]
    ):
        ...
        # Default to -1 (unknown / whole-trace) until a relevant step is found.
        # Using len(trace_steps) - 1 as the fallback wrongly implicates the
        # final step when no failing step was actually identified.
        failing_step_idx = -1
        original_prompt = ""
        original_response = ""

        for i, step in enumerate(trace_steps):
            if hasattr(step, "type") and step.type == StepType.llm_call:
                original_prompt = str(step.input.get("kwargs", {}).get("messages", ""))
                original_response = str(step.output.get("content", ""))
                failing_step_idx = i
        ...
        entry = FeedbackEntry(
            ...
            evidence=evidence,
            metadata={
                "autopsy_summary": autopsy_result.get("summary", ""),
    ...
    def _save_entry(self, entry: FeedbackEntry):
        ...
        safe_trace_id = entry.trace_id.replace("/", "_").replace("\\", "_").replace("..", "_")
        safe_step_idx = max(0, entry.step_index)
        filename = f"{safe_trace_id}_{safe_step_idx}.json"
    ...
    def export_fine_tuning_dataset(
        self, output_path: str, model_format: str = "openai"
    ) -> str:
                    ...
                    examples.append(
                        {
                            "messages": [
                                {
                                    "role": "system",
                                    "content": f"You are a helpful assistant. Avoid {entry.failure_type} errors.",
                                },
                                {"role": "user", "content": entry.original_prompt},
                                {
                                    "role": "assistant",
                                    "content": entry.corrected_response
                                    or entry.suggested_fix,
                                },
                            ]
                        }
                    )
