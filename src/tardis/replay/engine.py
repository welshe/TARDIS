...
class ReplayEngine:
    ...
    def replay(
        ...
        edit_tool_output: dict | None = None,
    ):
        ...
        replay_state = {"variables": {}, "context": {}, "step_count": 0}
        ...
        for s in self.trace.steps[from_idx:to_idx]:
            ...
            if s.index in self.breakpoints:
                ...
                if not sys.stdin.isatty():
                    print("  (non-interactive — skipping breakpoint)")
                    break
                response = input(
    ...
    def _replay_llm_call(self, step, output, state):
        ...
        print(
            f"      Cost: ${step.cost_usd:.6f}" if step.cost_usd else "      Cost: N/A"
        )
    ...
    def diff(self, other_trace_id: str):
        ...
        if not other:
            raise ValueError(f"Trace {other_trace_id} not found")
    ...
    def analyze_patterns(self):
        ...
        if llm_calls:
            ...
            if len(unique_hashes) < len(hashes):
                print(
                    f"  Repeated patterns detected: {len(hashes) - len(unique_hashes)} duplicates"
                )
        ...
        if tool_calls:
            ...
            for name, count in sorted(
                tool_names.items(), key=lambda x: x[1], reverse=True
            ):
                print(f"    {name}: {count}")
        ...
        if dom_snapshots or acc_snapshots:
            ...
            total_nodes = sum(
                _count_nodes(s.output.get("elements", {}))
                for s in dom_snapshots + acc_snapshots
            )
