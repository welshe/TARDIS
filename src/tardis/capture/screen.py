...
class ScreenBuffer:
    ...
    def _primary_monitor(sct):
        ...
        mss reports monitors[0] as the bounding box of all monitors and
        monitors[1..] as individual displays. Fall back to the largest
        individual monitor, and to monitors[0] if none exist, so single-monitor
        and unusual layouts don't crash on monitors[1] being absent.
        """
    ...
    def start(self):
        ...
        def loop():
            with mss() as sct:
                monitor = self._primary_monitor(sct)
                while self.running:
                    try:
    ...
    def compute_diff(self, frame1, frame2, threshold=30):
        ...
        change_percentage = (
            (changed_pixels / total_pixels) * 100 if total_pixels else 0.0
        )
    ...
    def get_movement_regions(self, frame1, frame2, threshold=30):
        ...
        def flood_fill(start_x, start_y):
            ...
            # +1 makes the box inclusive of both endpoints.
    ...
    def capture_with_metadata(self):
        ...
        with mss() as sct:
            shot = sct.grab(self._primary_monitor(sct))
