...
class ComplexityAnalyzer:
    ...
    def analyze(self, query: str) -> tuple[float, dict[str, Any]]:
        ...
        breakdown["keyword_score"] = min(
            1.0, keyword_matches / max(1, total_keywords) * 10
        )
...
class CostAwareRouter:
    ...
    def register_model(self, config: ModelConfig, client: Callable | None = None):
    ...
    def select_model(
        self, query: str, required_capabilities: list[str] | None = None
    ) -> RoutingDecision:
    ...
    async def route_and_execute(
        ...
        Requires model clients to be registered via register_model().
        Raises KeyError if no client is registered for the selected model.

        Args:
            query: The input query to route and execute.
            required_capabilities: Optional capability filter.

        Returns:
            Dict with response, routing decision, and cost data.

        Raises:
            KeyError: If no client is registered for the selected model.
        """
        ...
        if decision.selected_model not in self.model_clients:
            raise KeyError(
                f"No client registered for model '{decision.selected_model}'. "
                f"Register a client via register_model('{decision.selected_model}', client). "
        ...
        try:
            ...
            actual_tokens = getattr(usage, "total_tokens", 500) if usage else 500
    ...
    def _calculate_potential_savings(self) -> dict[str, Any]:
        ...
        expensive_model = next(
            (m for m in self.models.values() if m.tier == ModelTier.EXPENSIVE), None
        )
        ...
        actual_cost = sum(
            d.actual_cost or d.estimated_cost for d in self.routing_history
        )
        savings = always_expensive_cost - actual_cost
        savings_pct = (
            (savings / always_expensive_cost * 100) if always_expensive_cost > 0 else 0
        )
