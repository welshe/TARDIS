...
class SharedMemory:
    ...
    def put(self, key: str, value: Any, ttl: Optional[float] = None, namespace: str = "default") -> str:
    ...
    def get_meta(self, key: str, namespace: str = "default") -> Optional[dict]:
        full_key = f"{namespace}:{key}"
        with self._lock:
            entry = self._data.get(full_key)
            if entry is None:
    ...
    def cas(self, key: str, expected_version: int, new_value: Any, ttl: Optional[float] = None, namespace: str = "default") -> bool:
        full_key = f"{namespace}:{key}"
        with self._lock:
            entry = self._data.get(full_key)
    ...
    def snapshot(self) -> dict:
        with self._lock:
            return {
                "data": {k: {"value": v["value"], "version": v["version"]} for k, v in self._data.items()},
                "version": self._version,
            }
