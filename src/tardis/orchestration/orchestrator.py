...
def _drain_futures(
    ...
    """Collect any already-completed futures and record their outcomes."""
    if not futures:
        return
    done, _ = wait(list(futures.keys()), timeout=0, return_when=FIRST_COMPLETED)
    for future in done:
        task, agent = futures.pop(future)
        try:
            future.result()
            with lock:
...
class Orchestrator:
    ...
    _recorder: Any | None = field(default=None, repr=False)
    ...
    def submit(self, task: Task, fn: Callable | None = None):
        with self._lock:
    ...
    def _find_agent(self, task: Task) -> Agent | None:
        ...
        for agent in self.agents.values():
            if agent.state == AgentState.waiting and required.issubset(
                agent.capabilities
            ):
                return agent
        return None
        ...
        try:
            ...
            self._log_event(
                "task_completed",
                {
                    "task_id": task.id,
                    "agent_id": agent.id,
                    "duration_ms": int(task.duration_seconds * 1000)
    ...
    def run_parallel(
        self, fn_map: dict[str, Callable] = None, strict_capabilities: bool = False
    ) -> list[Task]:
        """
        ...
        while tasks_to_run or self._pending:
            if not tasks_to_run and self._pending:
                # All known tasks are queued (no idle agent). Drain whatever
                # work is in flight so freed agents can pick them up.
                _drain_futures(futures, self._completed, self._failed, self._lock)
                if not futures:
                    # Nothing in flight and still no idle agent: stop to avoid
                    # spinning forever; remaining tasks stay in _pending.
                    break
                continue
            ...
            # Atomic agent reservation: find + transition to running under lock.
            with self._lock:
                for a in self.agents.values():
                    if (
                        a.state == AgentState.idle
                        and required.issubset(a.capabilities)
                    ):
                        agent = a
                ...
                if agent is None and strict_capabilities:
                    ...
                    self._log_event(
                        "task_failed",
                        {"task_id": task.id, "reason": "no_capable_agent_strict_mode"},
                    )
                    continue
            ...
            if agent is None:
                # No idle agent right now. Re-queue and retry after in-flight
                # work frees up agents (handled by the drain above).
                with self._lock:
                    self._pending.append(task)
                self._log_event(
                    "task_queued", {"task_id": task.id, "reason": "no_idle_agent"}
                )
                continue
        ...
        # Final drain of any remaining futures.
    ...
    def run_sequential(self, fn_map: dict[str, Callable] = None) -> list[Task]:
        ...
        for task in tasks_to_run:
            ...
            if agent is None:
                self._log_event(
                    "task_skipped", {"task_id": task.id, "reason": "no_available_agent"}
                )
