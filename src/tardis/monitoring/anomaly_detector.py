...
class AnomalyDetector:
    ...
    def __init__(self, window_size: int = 20, z_threshold: float = 2.5, context_limit: int | None = None):
        ...
        # Token totals are tracked per-trace: summing across unrelated traces
        # would falsely report context overflow on a shared detector instance.
        self._total_tokens: dict[str, int] = defaultdict(int)
        self._context_limit = context_limit or 128000  # default GPT-4 Turbo context window
        self._last_dom_element_count: int | None = None
    ...
    def process_step(
        ...
        with self._lock:
            ...
            if step.type == StepType.llm_call:
                token_count = step.token_count.get("total_tokens", 0)
                latency = step.duration_ms or 0
                cost = step.cost_usd or 0.0
                self._total_tokens[trace_id] += token_count

                # Check for hash repetition (loop detection)
                response_hash = step.hash or ""
                if response_hash:
                    if response_hash in self._hash_history:
                        anomalies.append(
                            self._create_anomaly(
                                ...
                                step_index,
                                trace_id,
                                "LLM response hash repeated - possible infinite loop",
                                {
                                    "hash": response_hash,
                                    "history_count": len(self._hash_history),
                                },
                ...
                self._token_mean, self._token_std = self._update_stats(
                    self._token_counts
                )
                self._latency_mean, self._latency_std = self._update_stats(
                    self._latencies
                )
            ...
            elif step.type == StepType.dom_snapshot:
                # Check for grounding drift (layout shifts) by comparing the
                # element count of this snapshot against the previous one. The
                # previous LLM hash history is irrelevant here.
                output = step.output or {}
                # DOM snapshots from diff_snapshots() produce layout_shift (bool)
                # and layout_shift_count (int). Use available fields.
                elements = output.get("elements") or {}
                children = (
                    elements.get("children") if isinstance(elements, dict) else None
                )
                element_count = len(children) if isinstance(children, list) else 0

                prev_count = self._last_dom_element_count
                self._last_dom_element_count = element_count
                if prev_count is not None and prev_count > 0:
                    drift = abs(element_count - prev_count) / prev_count
                    if drift > 0.5:
                        anomalies.append(
                            self._create_anomaly(
                                ...
                                step_index,
                                trace_id,
                                f"DOM grounding drift of {drift:.0%} "
                                f"({prev_count} -> {element_count} elements)",
                                {
                                    "previous_element_count": prev_count,
                                    "element_count": element_count,
                                    "drift_ratio": drift,
                                },
                                "Investigate agent actions that mutate the "
                                "environment unexpectedly between snapshots",
                            )
                        )
            ...
            # Check context overflow risk (per-trace totals)
            total_tokens = self._total_tokens[trace_id]
            if total_tokens > self._context_limit * 0.8:
                severity = (total_tokens - self._context_limit * 0.8) / (
                    self._context_limit * 0.2
                )
                ...
                anomalies.append(
                    self._create_anomaly(
                        ...
                        step_index,
                        trace_id,
                        f"Context window at {total_tokens / self._context_limit:.1%} capacity",
                        {
    ...
    def _create_anomaly(
        ...
        final_severity = severity if severity is not None else base_severity
        final_severity = max(base_severity, final_severity)
    ...
    def reset(self):
        ...
        with self._lock:
            ...
            self._last_dom_element_count = None
        ...
        with self._lock:
            return {
                "token_stats": {
                    "mean": self._token_mean,
                    "std": self._token_std,
                    "window": len(self._token_counts),
                },
                "latency_stats": {
                    "mean": self._latency_mean,
                    "std": self._latency_std,
                    "window": len(self._latencies),
                },
                "cost_stats": {
                    "mean": self._cost_mean,
                    "std": self._cost_std,
                    "window": len(self._costs),
                },
                "consecutive_errors": self._consecutive_errors,
                "recent_tool_failures": self._recent_tool_failures,
                "total_tokens": sum(self._total_tokens.values()),
                "context_utilization": sum(self._total_tokens.values())
                / self._context_limit,
            }
