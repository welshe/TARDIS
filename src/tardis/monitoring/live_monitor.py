...
class LiveMonitor:
    ...
    def _on_anomaly(self, anomaly: AnomalyEvent):
        ...
        with self._lock:
            ...
            if len(self._anomaly_history) > self.config.max_history:
                self._anomaly_history = self._anomaly_history[-self.config.max_history:]
            
            # Generate alert if severity exceeds threshold
            if self.config.enable_alerts and anomaly.severity >= self.config.anomaly_threshold:
    ...
    def process_step(self, step: Step, trace_id: str, step_index: int) -> List[AnomalyEvent]:
        ...
        with self._lock:
            ...
            if step.metadata:
                self._total_tokens += step.metadata.get("total_tokens", 0)
                self._total_cost += step.metadata.get("cost", 0.0)
            
            if step.step_type.value == "error":
                self._error_count += 1
        
        return self.detector.process_step(step, trace_id, step_index)
        ...
        with self._lock:
            return {
                ...
                "alerts": {
                    ...
                    "unresolved": len([a for a in self._alerts if a.severity >= self.config.anomaly_threshold]),
