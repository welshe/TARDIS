...
class KnowledgeGraph:
    ...
    def find_similar(
        ...
        for node_id, node in self.nodes.items():
            ...
            query_keywords = set()
            for v in query_content.values():
                if isinstance(v, str):
                    query_keywords.update(v.lower().split())
            node_keywords = set()
    ...
    def get_related(self, node_id: str, depth: int = 1) -> list[KnowledgeNode]:
        ...
        Depth is capped at 10 to prevent DoS on large graphs.
        Cycle detection via visited set prevents infinite traversal.
        """
        if node_id not in self.nodes:
            return []

        effective_depth = min(depth, 10)
        visited = {node_id}
        queue = [(node_id, 0)]
        related = []
        ...
        while queue:
            ...
            for neighbor_id in self.adjacency_list.get(current_id, set()):
                if neighbor_id not in visited and neighbor_id in self.nodes:
                    ...
                    if current_depth + 1 < effective_depth:
    ...
    def record_fix_application(
        ...
        if edge_id not in self.edges:
            self.add_edge(
                ...
                fix_id,
                "fixes",
                metadata={"applications": 0, "successes": 0},
            )
        ...
        if success:
            self.nodes[fix_id].confidence_score = min(
                1.0, self.nodes[fix_id].confidence_score + 0.05
            )
            self.nodes[fix_id].usage_count += 1
            self.nodes[fix_id].last_used = time.time()
        else:
            self.nodes[fix_id].confidence_score = max(
                0.1, self.nodes[fix_id].confidence_score - 0.1
            )
        ...
        for neighbor_id in self.adjacency_list.get(failure_pattern_id, set()):
            if neighbor_id in self.nodes and self.nodes[neighbor_id].node_type == "fix":
                edge_id = self._generate_edge_id(
                    failure_pattern_id, neighbor_id, "fixes"
                )
                ...
                if edge_id in self.edges:
                    ...
                    combined_score = (
                        self.nodes[neighbor_id].confidence_score * success_rate
                    )
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        avg_confidence = sum(n.confidence_score for n in self.nodes.values()) / max(
            len(self.nodes), 1
        )
...
class SharedMemory:
    ...
    def link_fix_to_failure(
        self, failure_id: str, fix_id: str, success: bool = True
    ) -> None:
    ...
    def find_similar_failures(
        self, failure_description: str, tags: list[str]
    ) -> list[tuple[KnowledgeNode, float]]:
...
def create_shared_memory(agent_id: str | None = None) -> SharedMemory:
