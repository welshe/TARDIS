...
class Autopsy:
    ...
    def classify(self):
        ...
        if results:
            results.sort(key=lambda x: (_PATTERN_PRIORITY.get(x[0], 0), x[2]), reverse=True)
    ...
    def _check_grounding_failure(self):
        ...
        all_text = error_text + " " + str(self.trace.steps).lower()
        
        patterns = [
            "element not found", "elementnotfound", "click failed", "no such element",
    ...
    def _check_environment_drift(self):
        ...
        # Use structured field access instead of string serialization
        error_text = self._get_error_text()
        text = error_text + " " + str(self.trace.steps).lower()
        patterns = [
