...
def test_trace_get_steps_by_type():
    ...
    trace.add_step(Step(trace_id="test", index=2, type=StepType.llm_call, input={}, output={}))
...
def test_trace_error_tracking():
    ...
    # Add error step
    step2 = Step(trace_id="test", index=1, type=StepType.error, input={}, output={"error": "test"})
    trace.add_step(step2)
...
def test_trace_get_error_steps():
    ...
    trace = Trace()
    trace.add_step(Step(trace_id="test", index=0, type=StepType.llm_call, input={}, output={}))
    trace.add_step(Step(trace_id="test", index=1, type=StepType.error, input={}, output={"error": "test"}))
    trace.add_step(Step(trace_id="test", index=2, type=StepType.tool_call, input={}, output={}, success=False))
