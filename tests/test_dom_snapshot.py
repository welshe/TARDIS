...
def test_diff_snapshots_added():
    before = {"timestamp": 100.0, "method": "test", "elements": {"tag": "div", "id": "r"}}
    after = {
        "timestamp": 101.0,
        "method": "test",
        "elements": {"tag": "div", "id": "r", "children": [{"tag": "p", "text": "new"}]},
    }
...
def test_diff_snapshots_removed():
    before = {
        "timestamp": 100.0,
        "method": "test",
        "elements": {"tag": "div", "id": "r", "children": [{"tag": "p", "text": "old"}]},
    }
    after = {"timestamp": 101.0, "method": "test", "elements": {"tag": "div", "id": "r"}}
    diff = diff_snapshots(before, after)
    assert diff["added_count"] == 0
...
def test_grounding_check_with_snapshot_diff():
    ...
    snap1 = {
        "schema": "0.2.0", "timestamp": 100.0, "method": "test",
        "elements": {"tag": "div", "id": "r", "children": [
            {"tag": "button", "id": "btn", "rect": (10, 10, 100, 40)},
        ]},
    }
    snap2 = {
        "schema": "0.2.0", "timestamp": 101.0, "method": "test",
        "elements": {"tag": "div", "id": "r", "children": [
            {"tag": "button", "id": "btn", "rect": (300, 400, 100, 40)},
        ]},
    }

    trace.add_step(Step(trace_id="test", index=0, type=StepType.dom_snapshot,
                        input={}, output=snap1))
    trace.add_step(Step(trace_id="test", index=1, type=StepType.dom_snapshot,
                        input={}, output=snap2))

    # Add a tool error to ensure the trace is marked as failed
    trace.add_step(Step(trace_id="test", index=2, type=StepType.error,
                        input={}, output={"error": "ElementNotFound: button moved"}))

    autopsy = Autopsy(trace)
    failure_type, details, confidence = autopsy.classify()

    assert failure_type == FailureType.grounding_failure, f"Got {failure_type}: {details}"
    assert confidence >= 0.5
    evidence_keys = [e[0] for e in autopsy.evidence]
    assert any("layout" in k for k in evidence_keys), f"No layout evidence in {evidence_keys}"
